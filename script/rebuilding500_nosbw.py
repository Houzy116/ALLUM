...
if __name__=='__main__':
    ...
    def rebuilding_all(year,month,landcover01,landcover20):
        albedo=stat[f'{year}-{month}-albedo']
        c=str(month).zfill(2)
        snow_cover1=rasterio.open(root_path+f'snow/snow_monthly/2001{c}.tif').read(1)
        snow_cover2=rasterio.open(root_path+f'snow/snow_monthly/2020{c}.tif').read(1)
        snow_cover1[landcover01==15]=100
        snow_cover2[landcover20==15]=100
        snow_cover=np.maximum(snow_cover1,snow_cover2)
        del snow_cover1,snow_cover2
        gc.collect()
        if year==2020:
            landcover=landcover20
        else:
            landcover=landcover01
        ...
        for lat in tqdm(range(90,-90,-1), desc=f'{year}-{month}'):
            for lon in range(-180,180):
                ...
                if sza_L[year-2001,month-1,90-coord[1],coord[0]+180]==2:
                    ...
                    kk=k*(1-snow_cover[y:y+y_s,x:x+x_s]/100)
                    if sza_L[year-2001,month-1,90-coord[1],coord[0]+180]==1:
                        kk+=1
                albedo_500[y:y+y_s,x:x+x_s]=kk
        tif_save(albedo_500,f"/ssd_hzy/albedo_rebuilding/rebuilding2_albedo_{year}_{month}.tif",global_trf,p='4326')
    ...
    landcover01=rasterio.open(root_path+f"landcover/2001_landcover/2001_landcover.tif").read(1)
    landcover20=rasterio.open(root_path+f"landcover/2020_landcover/2020_landcover.tif").read(1) 
    for y in [2001,2020]:
        
        for m in range(1,13):
            rebuilding_all(y,m,landcover01,landcover20)
